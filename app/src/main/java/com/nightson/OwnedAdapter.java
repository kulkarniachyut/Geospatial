...
public class OwnedAdapter extends BaseAdapter {
    ...
    public OwnedAdapter(Context mContext, ArrayList<OwnedEventObject> mOwnedObjectList , boolean isOwned) {
    ...
    public void updateEvent(String name,String starttime ,String endtime , String latitude, String longitude, String id) {
        ...
        SharedPreferences preferences = mContext.getSharedPreferences(Constants.PREF_FILE_NAME,0);
...
//                mListView.setAdapter(kAdapter);
        ...
        UpdateEventsRequest updateEventsRequest = new UpdateEventsRequest(name,String.valueOf(startTime),String.valueOf(endTime),latitude,longitude,id,
    ...
    private void callPopup(LayoutInflater layoutInflater, final OwnedEventObject mObject, final int position) {

        if(isOwned) {
            View popupView = layoutInflater.inflate(R.layout.event_popup1, null);
            ...
            final EditText mname = (EditText) popupView.findViewById(R.id.ownedname1);
            final EditText mstart = (EditText) popupView.findViewById(R.id.starttime1);
            final EditText mend = (EditText) popupView.findViewById(R.id.endtime1);
            final EditText mlat = (EditText) popupView.findViewById(R.id.latitude);
            final EditText mlong = (EditText) popupView.findViewById(R.id.longitude);
...
//                         popupWindow.dismiss();
                            ...
                            try {
                                date = format.parse(mstart.getText().toString().split("e :")[1].trim());
                                date1 = format.parse(mend.getText().toString().split("e :")[1].trim());
                            }catch (Exception e){
                                e.printStackTrace();
                            }


                            mOwnedEventObjects.get(position).mName = mname.getText().toString().split(":")[1];
                            mOwnedEventObjects.get(position).mStartDate = date;
                            mOwnedEventObjects.get(position).mEndTime = date1;
                            mOwnedEventObjects.get(position).mlatitude = mlat.getText().toString();
                            mOwnedEventObjects.get(position).mlongitude = mlong.getText().toString();



                            updateEvent(mname.getText().toString().split(":")[1],mstart.getText().toString().split("e :")[1],
                                    mend.getText().toString().split("e :")[1],mlat.getText().toString().split(":")[1],
                                    mlong.getText().toString().split(":")[1],mObject.getmEventId());
